...
def run_thread(thread_state: dict) -> None:
    ...
    while True:
        ...
        if open_pos is not None:
            # only check TP/SL, don't ask AI again until it's closed
            closed = db.check_position_tp_sl(open_pos["id"], candle)
            if closed:
                thread_state["open_position_id"] = None
        else:
            # no open position -> ask AI for a fresh signal
            fetcher.fetch_data_for_back_test(candle["Timestamp"], thread_index)

            result = deepseek_client.run_from_file(config.BACK_TEST_OUTPUT_FILES[thread_index])
            raw, chat_link = result["response"], result["chat_link"]
